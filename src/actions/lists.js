...
export const create_list = (
    ...
    list_author
        ...
        try {
            const res = await axios.post(`${env.DATABASE_URL}/api/lists/`, body, config)
...
export const load_user_lists = (user_id) => async dispatch => {
    ...
    try {
        const res = await axios.get(`${env.DATABASE_URL}/api/lists/user/${user_id}/`, config)
...
export const delete_list = (list_id) => async dispatch => {
    ...
    try {
        const res = await axios.delete(`${env.DATABASE_URL}/api/lists/${list_id}/`);
...
export const update_list = (list_id, list_title, list_entry_1, list_entry_2, list_entry_3, list_entry_1_desc, list_entry_2_desc, list_entry_3_desc, list_author) => async dispatch => {
    ...
    try {
        const res = await axios.patch(`${env.DATABASE_URL}/api/lists/${list_id}/`, body, config);
...
export const load_list_owner = (owner_id) => async dispatch => {
    ...
    try {
        const res = await axios.get(`${env.DATABASE_URL}/api/lists/owner/${owner_id}/`, config);
